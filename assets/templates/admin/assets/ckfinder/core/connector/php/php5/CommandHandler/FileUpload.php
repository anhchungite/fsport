...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        while (true)
        {	
        	$name = convertUrl($uploadedFile['name']);
        	if(strlen($name) > 50){
        		$name = substr($name, 0, 50);
        	}else {
        		$arr_name = explode('.', $uploadedFile['name']);
        		array_pop($arr_name);
        		$name = convertUrl(implode('_', $arr_name));
        	}
        	
        	$path_parts = pathinfo($sFileName);
        	$sFileName = $name.'_'.uniqid('tac_') . '.' . $path_parts['extension'];
